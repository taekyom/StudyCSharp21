...
namespace AddressBookApp
{
    class AddressManager
    {
        ...
        public int SelectMenu()
        {
            ...
            string input = Console.ReadLine();         // ReadLine() : 키보드입력을 문자열로 받음.
            int.TryParse(input, out int menuNum);      // 키보드로 입력받은 문자열을 정수로 변환

            if (!(menuNum > 0 && menuNum < 7))  // 키보드로 입력받은 값이 1~6이 아닌 경우
            {
                menuNum = 0;
            }
            return menuNum;
        ...
        public void SearchAddress()
        {
            ...
            foreach (var item in listaddress)
            {
                if (item.Name == name)
                {
                    isFind = true; // 찾는 이름과 일치하는 이름이 있을 때
                    Console.WriteLine();
